...
class HybridYOLOCamShift:
    def __init__(self, video_name, frames_dir="my_frames", labels_file="manual_labels.json", model_version="n", class_id=0):
        ...
        self.results_dir = os.path.join("experiments", video_name, "results", "hybrid_camshift", f"hybrid_camshift_{model_version}")
    ...
    def draw_result(self, frame, ground_truth, yolo_box, cs_box, winner_box, winner_name):
        ...
        if yolo_box: # YOLO (синий)
            ...
            cv2.rectangle(result_frame,
                          ...
                          (255, 0, 0), 2)
        if cs_box and cs_box[2] > 1 and cs_box[3] > 1: # CamShift (жёлтый)
            x_c, y_c, w_c, h_c = cs_box
            cv2.rectangle(result_frame,
        ...
        if winner_box: # Победитель (красный)
            ...
            cv2.rectangle(result_frame,
                          ...
                          (0, 0, 255), 3)
        y_offset = 30
        cv2.putText(result_frame, f"HYBRID: YOLOv8{self.model_version} + CamShift", (10, y_offset),
    ...
    def run(self):
        print(f"=== ГИБРИДНАЯ СИСТЕМА: YOLOv8{self.model_version} + CamShift ===")
        print("=" * 60)
        ...
        cs_tracker.init(first_frame, tuple(map(int, first_box)))
        cs_box = first_box
        print(f"Инициализация CamShift по YOLO (max IoU): {first_box}")
        ...
        for i, frame_path in enumerate(all_frames):
            ...
            if cs_box is not None:
                ...
                if results and len(results) > 0 and results[0].boxes is not None:
                    ...
                    yolo_box = best_box
                    yolo_conf = best_conf
            # 2. Обновление CamShift
            cs_success, cs_box_new = cs_tracker.update(frame)
            if cs_success and cs_box_new is not None:
                cs_box = cs_box_new
            else:
                cs_box = None
            ...
            elif cs_box is not None:
                winner_box = cs_box
                winner_name = "CamShift"
            ...
            if ground_truth is not None:
                ...
                cs_iou = self.calculate_iou(ground_truth, cs_box) if cs_box else 0
            else:
                iou, cle, yolo_iou, cs_iou = 0, float('inf'), 0, 0
            # Остановка при потере целевого объекта
            if cs_iou < 0.1 and cs_box is not None:
            ...
            # Визуализация
            vis_frame = self.draw_result(frame, ground_truth, yolo_box, cs_box, winner_box, winner_name)
            result_path = os.path.join(self.results_dir, f"result_{frame_name}")
            cv2.imwrite(result_path, vis_frame)
            cv2.imshow(f'Hybrid YOLOv8{self.model_version}+CamShift', vis_frame)
    ...
    def print_statistics(self):
        if not self.results:
            ...
            return
        print("\n" + "=" * 50)
        print(f"СТАТИСТИКА ГИБРИДНОЙ СИСТЕМЫ (YOLOv8{self.model_version}+CamShift)")
        print("=" * 50)
        ...
        cs_wins = sum(1 for r in self.results if r["winner"] == "CamShift")
        print(f"Всего кадров: {len(self.results)}")
        print(f"Побед YOLO: {yolo_wins} ({yolo_wins / len(self.results) * 100:.1f}%)")
        print(f"Побед CamShift: {cs_wins} ({cs_wins / len(self.results) * 100:.1f}%)")
        print(f"Средний IoU (гибрид): {np.mean(ious):.3f}")
...
def main():
    ...
    print("Запуск гибридной системы YOLO + CamShift")
    print("=" * 50)
    versions = [args.model] if args.model else ['n', 's', 'm', 'l', 'x']
    for v in versions:
        print(f"\n\n{'='*50}")
        print(f"Тестирование модели YOLOv8{v}")
        hybrid = HybridYOLOCamShift(video_name=args.video, model_version=v, class_id=args.class_id)
        hybrid.run()
