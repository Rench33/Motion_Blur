...
class HybridYOLOOF:
    def __init__(self, video_name, frames_dir="my_frames", labels_file="manual_labels.json", model_version="n", class_id=0):
        ...
        self.results_dir = os.path.join("experiments", video_name, "results", "hybrid_of", f"hybrid_of_{model_version}")
    ...
    def draw_result(self, frame, ground_truth, yolo_box, of_box, winner_box, winner_name):
        ...
        if yolo_box: # YOLO (синий)
            ...
            cv2.rectangle(result_frame,
                          ...
                          (255, 0, 0), 2)
        if of_box and of_box[2] > 1 and of_box[3] > 1: # Optical Flow (жёлтый)
            x_c, y_c, w_c, h_c = of_box
            cv2.rectangle(result_frame,
        ...
        if winner_box: # Победитель (красный)
            ...
            cv2.rectangle(result_frame,
                          ...
                          (0, 0, 255), 3)
        y_offset = 30
        cv2.putText(result_frame, f"HYBRID: YOLOv8{self.model_version} + Optical Flow", (10, y_offset),
    ...
    def run(self):
        print(f"=== ГИБРИДНАЯ СИСТЕМА: YOLOv8{self.model_version} + Optical Flow ===")
        print("=" * 60)
        ...
        of_tracker.init(first_frame, tuple(map(int, first_box)))
        of_box = first_box
        print(f"Инициализация Optical Flow по YOLO (max IoU): {first_box}")
        ...
        for i, frame_path in enumerate(all_frames):
            ...
            if of_box is not None:
                ...
                if results and len(results) > 0 and results[0].boxes is not None:
                    ...
                    yolo_box = best_box
                    yolo_conf = best_conf
            # 2. Обновление Optical Flow
            of_success, of_box_new = of_tracker.update(frame)
            if of_success and of_box_new is not None:
                of_box = of_box_new
            else:
                of_box = None
            ...
            elif of_box is not None:
                winner_box = of_box
                winner_name = "OpticalFlow"
            ...
            if ground_truth is not None:
                ...
                of_iou = self.calculate_iou(ground_truth, of_box) if of_box else 0
            else:
                iou, cle, yolo_iou, of_iou = 0, float('inf'), 0, 0
            # Остановка при потере целевого объекта
            if of_iou < 0.1 and of_box is not None:
            ...
            # Визуализация
            vis_frame = self.draw_result(frame, ground_truth, yolo_box, of_box, winner_box, winner_name)
            result_path = os.path.join(self.results_dir, f"result_{frame_name}")
            cv2.imwrite(result_path, vis_frame)
            cv2.imshow(f'Hybrid YOLOv8{self.model_version}+OpticalFlow', vis_frame)
    ...
    def print_statistics(self):
        if not self.results:
            ...
            return
        print("\n" + "=" * 50)
        print(f"СТАТИСТИКА ГИБРИДНОЙ СИСТЕМЫ (YOLOv8{self.model_version}+OpticalFlow)")
        print("=" * 50)
        ...
        of_wins = sum(1 for r in self.results if r["winner"] == "OpticalFlow")
        print(f"Всего кадров: {len(self.results)}")
        print(f"Побед YOLO: {yolo_wins} ({yolo_wins / len(self.results) * 100:.1f}%)")
        print(f"Побед Optical Flow: {of_wins} ({of_wins / len(self.results) * 100:.1f}%)")
...
def main():
    ...
    print("Запуск гибридной системы YOLO + Optical Flow")
    print("=" * 50)
    versions = [args.model] if args.model else ['n', 's', 'm', 'l', 'x']
    for v in versions:
        print(f"\n\n{'='*50}")
        print(f"Тестирование модели YOLOv8{v}")
        hybrid = HybridYOLOOF(video_name=args.video, model_version=v, class_id=args.class_id)
        hybrid.run()
